...
@cython.boundscheck(False)
def avg_xyz_frame_concat_trajs(target, reference, int frame=0, atom_indices=None,
         ref_atom_indices=None, bool parallel=True, bool precentered=False, weight=None, trajectory_prealigned = True):
    """avg_xyz_frame_est(target, reference, int frame=0, atom_indices=None,
       ref_atom_indices=None, bool parallel=True, bool precentered=False, weight=None, trajectory_prealigned = True)
	Compute average coordinates of bunch of frames or from a single trajectory given weights or None if treat all frames are equally.
	Parameters
    ----------
    target : md.Trajectory
    ...
    weight : 1D-array or ndarray shaped (n_trajectories, )
        Weight for each frame.
    trajectory_prealigned : bool, default = True
        Should provide aligned trajectories for RMSF caluclations.

    Returns
    -------
    average coordinates : np.ndarray, shape=(n_selected_atoms, 3)
        A ndarray of coordinates of selected atoms for RMSF calculation.
    """
    cdef bool atom_indices_is_none = False
    ...
    if parallel:
        for j in prange(n_atoms, nogil=True):
            for i in range(target_n_frames):
                avg_xyz_frame[j, 0] += target_displaced_xyz[i, j, 0] * cweights[i]
                avg_xyz_frame[j, 1] += target_displaced_xyz[i, j, 1] * cweights[i]
                avg_xyz_frame[j, 2] += target_displaced_xyz[i, j, 2] * cweights[i]
    else:
        for i in range(target_n_frames):
    ...
    #Return Average coordinates of selected atoms over a trajectory
    return avg_xyz_frame

@cython.boundscheck(False)
def rmsf_weight(target, reference, avg_xyz_frame_full, int frame=0, atom_indices=None, 
         ref_atom_indices=None, bool parallel=True, bool precentered=False, weight = None, trajectory_prealigned = True):
    """rmsf_weight(target, reference, avg_xyz_frame_full, frame=0, atom_indices=None, parallel=True, 
    precentered=False, weight = None, trajectories_n_frames = None)
    ...
    avg_xyz_frame_full : md.avg_xyz_frame_concat_trajs
    	The average coordinates of all selected atoms over the full trajectories. 
    frame : int, default=0
        The index of the conformation in `reference` to measure
        distances to.
    atom_indices : array_like, or None
    ...
    --------
    >>> import mdtraj as md                                      # doctest: +SKIP
    >>> rmsf = md.rmsf_weight(trajectory, trajectory, avg_xyz_frame, 0, weight)                # doctest: +SKIP
    ...
    The calculation is slightly faster if you precenter the trajectory
    >>> trajectory.center_coordinates()
    >>> rmsf = md.rmsf_weight(trajectory, trajectory, avg_xyz_frame, 0, precentered=True, weight)
    ...
    >>> rmsf = md.rmsf_weight(trajectory, None, avg_xyz_frame, atom_indices=rmsf_atom_indices, weight)
    ...
    # only extract the `frame`-th conformation from ref_xyz
    ref_xyz_frame = np.asarray(reference.xyz[frame, ref_atom_indices, :], order='C', dtype=np.float32)
    avg_xyz_frame = np.copy(avg_xyz_frame_full)
        ...
        for j in prange(n_atoms, nogil=True):
            for i in range(target_n_frames):
                fluctuations[j] += ((target_displaced_xyz[i, j, 0] - avg_xyz_frame[j, 0])**2 +
                                    ...
                                    (target_displaced_xyz[i, j, 2] - avg_xyz_frame[j, 2])**2) * cweights[i]
    else:
        for i in range(target_n_frames):
            for j in range(n_atoms):
                fluctuations[j] += ((target_displaced_xyz[i, j, 0] - avg_xyz_frame[j, 0])**2 +
    ...
    print ('RMSF is not done yet, you need to SQRT the resulting fluctuations')
